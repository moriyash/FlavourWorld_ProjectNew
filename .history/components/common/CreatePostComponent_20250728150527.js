...
const CreatePostComponent = ({ 
  ...
  // 🔧 בדיקה שהערכים קיימים ברשימות המותרות
  ...
  if (meatType && !validMeatTypes.includes(meatType)) {
    newErrors.meatType = 'Please select a valid meat type';
  }
  ...
  try {
    ...
    // 🔧 וידוא קטגוריה תקינה
    const validCategory = validCategories.includes(category) ? category : 'Asian';
    const validMeatType = validMeatTypes.includes(meatType) ? meatType : 'Mixed';
    
    console.log('🔧 Validated values:', {
      originalCategory: category,
      validCategory: validCategory,
      originalMeatType: meatType,
      validMeatType: validMeatType,
    ...
    const recipeData = {
      ...
      category: validCategory, // 🔧 ערך מוודא
      meatType: validMeatType, // 🔧 ערך מוודא
      prepTime: Math.max(0, totalMinutes), // 🔧 וידוא שהערך לא שלילי
      servings: Math.max(1, parseInt(servings) || 1), // 🔧 וידוא שהערך לפחות 1
    ...
    if (result && result.success) {
      ...
      console.log('❌ Recipe creation failed:', result?.message);
