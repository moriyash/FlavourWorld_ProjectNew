...
const PostComponent = ({ 
  ...
  // 🔍 הוסף את זה כאן לבדיקה:
  console.log('🎬 Post data:', {
    title: safePost.title,
    mediaType: safePost.mediaType,
    hasImage: !!safePost.image,
    hasVideo: !!safePost.video,
    image: safePost.image ? 'exists' : 'null',
    video: safePost.video ? 'exists' : 'null'
  });
  ...
  const renderMedia = () => {
    ...
    console.log('🎥 Rendering media:', { mediaType, hasImage, hasVideo });
  ...
  const renderFullRecipeMedia = () => {
    ...
    if ((mediaType === 'image' || hasImage) && !hasVideo) {
      return <Image source={{ uri: safePost.image }} style={styles.fullRecipeImage} />;
    }
