...
const PostComponent = ({ 
  ...
  try {
    ...
    formData.append('instructions', editData.instructions);
    formData.append('category', editData.category);
    formData.append('meatType', editData.meatType);
    formData.append('prepTime', editData.prepTime.toString());
    formData.append('servings', editData.servings.toString());
    formData.append('userId', currentUserId);
    formData.append('mediaType', editMediaType);

    // 🆕 הוספת קבצי מדיה אם הם חדשים (URI מקומי)
    if (editData.image && editData.image.startsWith('file://')) {
      ...
      formData.append('image', editData.image); // תמונה קיימת
    }

    if (editData.video && editData.video.startsWith('file://')) {
      formData.append('video', {
        uri: editData.video,
        type: 'video/mp4',
        name: 'video.mp4'
      });
    } else if (editData.video) {
      formData.append('video', editData.video); // וידאו קיים
    }

    let result;
    
    if (isActualGroupPost && effectiveGroupId) {
      result = await groupService.updateGroupPost(effectiveGroupId, postId, formData);
    } else {
      result = await recipeService.updateRecipe(postId, formData);
    }
