...
import { Video } from 'expo-av'; // 🆕 הוסף אימפורט לוידאו
...
const SharePostComponent = ({ 
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      ...
      try {
        ...
        // מיון לפי פעילות אחרונה ואז לפי סטטוס אונליין
        const sortedContacts = allContacts.sort((a, b) => {
          // תחילה משתמשים מחוברים
          if (a.isOnline && !b.isOnline) return -1;
          if (!a.isOnline && b.isOnline) return 1;
          
          // אחר כך לפי פעילות אחרונה
  ...
  const handleShare = async () => {
    ...
    try {
        ...
        try {
          ...
          if (!chatId) {
            ...
            if (createChatResult.success) {
              ...
              console.error(`Failed to create chat with ${contact.name}`);
              continue;
            }
          }
          ...
          const shareMessage = {
            ...
            sharedPost: {
              ...
              authorName: post?.user?.fullName || post?.user?.name || 'Unknown Chef',
      ...
      Alert.alert(
        'Recipe Shared! 🍳', 
        `Your delicious recipe has been shared with ${selectedContacts.length} ${selectedContacts.length === 1 ? 'person' : 'people'}!`
      );
