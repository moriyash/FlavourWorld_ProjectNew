...
const SharePostComponent = ({ 
  ...
  const [isSharing, setIsSharing] = useState(false); // 🆕 מצב שיתוף נפרד
      ...
      try {
        ...
        if (currentUserId) {
          try {
            ...
            console.warn('⚠️ Socket initialization failed, continuing without real-time features:', socketError);
          }
        }
        ...
        try {
          ...
          console.log('⚠️ Could not load following list:', followingError);
        }
  ...
  const handleShare = async () => {
    ...
    setIsSharing(true); // 🆕 שימוש במצב שיתוף נפרד
    ...
    try {
      ...
      for (const contact of selectedContactsData) {
        try {
          ...
          const messageContent = message && message.trim() 
            ? message.trim() 
            : `Check out this amazing recipe: ${post?.title || 'Delicious Recipe'}! 🍳`;

          // 🆕 שימוש בפונקציה הפשוטה יותר של שליחת הודעה
          const shareMessage = {
            chatId: chatId,
          ...
          // 🆕 שימוש בגרסה המתוקנת של sendMessage
        ...
        // 🆕 השהיה קטנה בין הודעות למניעת עומס
        await new Promise(resolve => setTimeout(resolve, 100));
      }

      // 🆕 קריאה חזרה למידע שיתוף רגיל - רק אם הפונקציה קיימת
      if (onShare && typeof onShare === 'function') {
        ...
        try {
          ...
          console.warn('⚠️ Error in onShare callback:', onShareError);
        }
      }
      ...
      if (successCount > 0 && failCount === 0) {
        ...
        // 🆕 ניווט למסך הצ'אט האחרון שנשלח אליו (אופציונלי)
        ...
        Alert.alert(
          'Share Failed', 
          'Failed to share recipe with selected contacts. Please try again.',
          [
      ...
      Alert.alert(
        'Error', 
        'Failed to share recipe. Please check your internet connection and try again.',
        [
          { text: 'OK', style: 'default' },
          { 
            text: 'Retry', 
  ...
  const renderContactItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity 
        ...
        disabled={isSharing} // 🆕 השבתה בזמן שיתוף
      >
    ...
    >
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity 
            onPress={handleShare} 
            disabled={selectedContacts.length === 0 || isSharing} // 🆕 שימוש במצב החדש
            style={[
              styles.shareButton,
              (selectedContacts.length === 0 || isSharing) && styles.disabledButton
            ]}
          >
            {isSharing ? (
        ...
        <View style={styles.messageContainer}>
          <TextInput
            ...
            multiline
            maxLength={200}
            editable={!isSharing} // 🆕 השבתה בזמן שיתוף
          />
        </View>
        ...
        <View style={styles.contactsContainer}>
          <View style={styles.contactsHeader}>
            ...
            <TouchableOpacity 
              ...
              disabled={loading || isSharing} // 🆕 השבתה בזמן שיתוף
            >
              <Text style={[
                styles.selectAllText,
                (loading || isSharing) && styles.disabledText
              ]}>
          ...
          <View style={styles.searchContainer}>
            ...
            <TextInput
              ...
              editable={!loading && !isSharing} // 🆕 השבתה בזמן שיתוף
            />
          </View>
          ...
          {loading ? (
            ...
            <FlatList
              ...
              scrollEnabled={!isSharing} // 🆕 השבתת גלילה בזמן שיתוף
              ListEmptyComponent={
        ...
        {/* 🆕 אינדיקטור שיתוף נוסף */}
        {isSharing && (
          <View style={styles.sharingOverlay}>
            <View style={styles.sharingIndicator}>
              <ActivityIndicator size="large" color={FLAVORWORLD_COLORS.primary} />
              <Text style={styles.sharingText}>Sharing your delicious recipe...</Text>
              <Text style={styles.sharingSubtext}>Please wait</Text>
            </View>
          </View>
        )}
      </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  // 🆕 סטיילים חדשים לאינדיקטור השיתוף
  sharingOverlay: {
    position: 'absolute',
    top: 0,
    left: 0,
