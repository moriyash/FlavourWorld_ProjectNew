...
const SimpleIcon = ({ type, size = 24, color = FLAVORWORLD_COLORS.text }) => {
  ...
  // תיקון: וידוא שהערך תמיד קיים ותקין
          ...
          chatContacts = chatsResult.data
            ...
            .map(chat => {
              ...
              if (otherUser) {
                return {
                  id: otherUser.userId,
                  name: otherUser.userName || 'Unknown', // תיקון: ערך ברירת מחדל
        ...
        try {
          ...
          if (followingResult.success && followingResult.data) {
            followingContacts = followingResult.data.map(user => ({
              id: user.id || user._id,
              name: user.fullName || user.name || 'Unknown', // תיקון: ערך ברירת מחדל
    ...
    try {
      ...
      for (const contact of selectedContactsData) {
        try {
          ...
          const shareMessage = {
            ...
            metadata: {
              sharedPost: {
                ...
                title: post?.title || 'Untitled Recipe', // תיקון: ערך ברירת מחדל
          ...
          console.log(`📨 Sending message to chat ${chatId}:`, shareMessage);
