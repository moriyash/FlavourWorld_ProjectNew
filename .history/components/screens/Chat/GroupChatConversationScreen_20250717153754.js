...
const GroupChatConversationScreen = ({ route, navigation }) => {
  ...
  const [typingUsers, setTypingUsers] = useState([]); // רשימת משתמשים שמקלידים
  ...
  useEffect(() => {
    ...
    // Socket listeners for loaded messages and group updates
    ...
    if (socket) {
      ...
      socket.on('group_chat_info_updated', (updatedChatInfo) => {
        if (updatedChatInfo._id === chatId) {
          setChatInfo(updatedChatInfo);
        }
      });

      socket.on('group_messages_marked_read', (data) => {
      ...
      socket.on('group_participant_joined', (data) => {
        if (data.chatId === chatId) {
          // Add system message
          const systemMessage = {
      ...
      socket.on('group_participant_left', (data) => {
        if (data.chatId === chatId) {
          // Add system message
          const systemMessage = {
            _id: Date.now().toString(),
      ...
      // הוסף את זה אחרי socket.on('group_participant_left', ...)
    ...
    return () => {
      ...
      if (socket) {
        ...
        socket.off('group_chat_info_updated');
        socket.off('group_messages_marked_read');
        socket.off('group_participant_joined');
        socket.off('group_participant_left');
      }
    };
  }, [chatId, currentUser]);
  ...
  const loadChatInfo = async () => {
    try {
      // Use socket-based loading through chat service
      chatService.loadGroupChatInfo(chatId);
      
      // For fallback, also try HTTP if socket fails
  ...
  const markAsRead = () => {
    chatService.markAsRead(chatId, 'group');
  };
