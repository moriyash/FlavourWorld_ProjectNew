...
const HomeScreen = ({ navigation }) => {
  ...
  console.log('🗑️ handlePostDelete called with:', postId); // <-- הוסף
  ...
  try {
    console.log('🔍 About to call recipeService.deleteRecipe...'); // <-- הוסף
    ...
    console.log('📊 Delete result:', result); // <-- הוסף
    
    if (result.success) {
      console.log('✅ Delete successful!'); // <-- הוסף
      handleRefreshData();
      Alert.alert('Success', 'Recipe deleted successfully');
    } else {
      console.log('❌ Delete failed:', result.message); // <-- הוסף
      Alert.alert('Error', result.message || 'Failed to delete recipe');
    }
  } catch (error) {
    console.error('💥 Delete error:', error); // <-- הוסף
