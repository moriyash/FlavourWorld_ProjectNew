...
// התחלת הקלדה בקבוצת צ'אט
socket.on('start_group_typing', (data) => {
  const { chatId, userId, userName } = data;
  socket.to(chatId).emit('typing_started', { chatId, userId, userName });
  console.log(`User ${userId} (${userName}) started typing in group ${chatId}`);
});

// עצירת הקלדה בקבוצת צ'אט
socket.on('stop_group_typing', (data) => {
  const { chatId, userId } = data;
  socket.to(chatId).emit('typing_stopped', { chatId, userId });
  console.log(`User ${userId} stopped typing in group ${chatId}`);
});
