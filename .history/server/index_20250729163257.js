...
const RecipeSchema = new mongoose.Schema({
  ...
  prepTime: {
    type: Number,
    required: true,
    min: 0 // 🔧 שינוי מ-1 ל-0 כדי לאפשר "0 דקות"
  },
  servings: {
    type: Number,
    required: true,
  ...
  // 🔧 שינוי userId לתמיכה גם ב-String וגם ב-ObjectId
  userId: {
    type: String, // שונה מ-ObjectId ל-String
    required: true
  },
  userName: {
    type: String,
  ...
  try {
    ...
    if (req.files && req.files.length > 0) {
      ...
      if (videoFile) {
        ...
        imageData = null; // אם יש וידאו חדש, מוחקים תמונה ישנה
        mediaType = 'video';
        console.log('🎥 Video updated, length:', videoData.length);
      } else {
        // 📸 חיפוש קובץ תמונה חדש
        const imageFile = req.files.find(file =>
          file.fieldname === 'image' ||
          file.mimetype.startsWith('image/')
        );
        ...
        if (imageFile) {
          ...
          const base64Image = imageFile.buffer.toString('base64');
          imageData = `data:${imageFile.mimetype};base64,${base64Image}`;
          videoData = null; // אם יש תמונה חדשה, מוחקים וידאו ישן
          mediaType = 'image';
          console.log('📸 Image updated, length:', imageData.length);
        }
      }
    } else {
      // 🔄 בדיקה אם יש מדיה חדשה בטופס
...
app.post('/api/recipes', upload.any(), async (req, res) => {
  try {
    ...
    // 🔧 ולידציה של category
    const category = formData.category || 'Asian'; // ברירת מחדל
    ...
    // 🔧 ולידציה של meatType
    const meatType = formData.meatType || 'Mixed'; // ברירת מחדל
    ...
    // 🔧 ולידציה של prepTime ו-servings
    ...
    // 🔄 תמיכה לאחור - אם אין קבצים אבל יש מדיה בטופס
    ...
    // 🔧 חיפוש משתמש רק אם יש userId תקין
...
app.get('/api/recipes/:id', async (req, res) => {
  try {
    ...
    const enrichedRecipe = {
      ...
      userAvatar: user ? user.avatar : null,
      userBio: user ? user.bio : null,
      // 🆕 וידוא שיש שדות מדיה (תאימות לאחור)
...
app.delete('/api/recipes/:id', async (req, res) => {
  try {
    ...
    // 🔍 תחילה נמצא את המתכון כדי לבדוק מה אנחנו מוחקים
