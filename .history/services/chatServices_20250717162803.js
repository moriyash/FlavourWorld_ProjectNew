...
const SOCKET_SERVER_URL = 'http://172.20.10.2:3000'; //תקשורת בזמן אמת
...
class ChatService {
  ...
  startTyping(chatId, chatType = 'private') {
    ...
    console.log(` Starting typing in ${chatType} chat:`, chatId);
    this.socket.emit(event, {
      chatId,
      userId: this.getCurrentUserId()
    });
  ...
  stopTyping(chatId, chatType = 'private') {
    ...
    console.log(` Stopping typing in ${chatType} chat:`, chatId);
    this.socket.emit(event, {
      chatId,
      userId: this.getCurrentUserId()
    });
