...
export const recipeService = {
  ...
  // פונקציה מתוקנת עם שם נכון וטיפול שגיאות משופר
deleteRecipe: async (recipeId, postData = null) => {
  try {
    ...
    // טיפול בפוסט קבוצה - אופציה 1: groupId ישיר
    ...
    // טיפול בפוסט קבוצה - אופציה 2: זיהוי אוטומטי
    if (postData && (postData.isGroupPost || postData.group)) {
      ...
      if (groupId) {
        ...
        try {
          ...
          console.warn('Group error details:', groupError.response?.data);
        }
      }
    }
    ...
    console.log('📍 API endpoint:', `${API_BASE_URL}/recipes/${recipeId}`);
    ...
    if (error.response) {
      ...
      console.error(`Server responded with ${status}:`, errorMessage);
...
// פונקציה משופרת עם טיפול שגיאות מפורט
deletePost: async (postId, postData) => {
  try {
    ...
    console.log('🔍 Post type detected:', isGroupPost ? 'GROUP' : 'REGULAR');
    ...
    if (isGroupPost) {
      ...
      console.log('📍 API endpoint:', `${API_BASE_URL}/recipes/${postId}`);
